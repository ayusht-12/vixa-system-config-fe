...
interface HsmPageHeaderProps {
  ...
  onExportKeysCsv?: () => void;
  onInitiateCeremony?: () => void;
}

export function HsmPageHeader({
  serial,
  badges,
  onExportAttestation,
  onExportKeysCsv,
      ...
      <div>
        <div className="flex items-center gap-3 mb-1 flex-wrap">
          ...
          {badges.map((badge) => (
            <span
              key={badge}
              className="px-2 py-0.5 rounded-small text-xs font-medium bg-[#0D0A1A] text-purple-400 border border-purple-500/25"
            >
              {badge}
            </span>
          ))}
