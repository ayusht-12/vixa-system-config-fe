...
export interface DashboardSummaryDTO {
  ...
  configurations_with_pending_changes: number;
  audit_events_last_24h: number;
  critical_audit_events_last_24h: number;
...
export interface TenantHealthDTO {
  ...
  critical_audit_event_count: number;
  open_anomaly_count: number;
}

export interface TenantHealthListDTO {
  items: TenantHealthDTO[];
  total: number;
}

export interface EventTrendBucketDTO {
  bucket: string;
  severity: string;
  count: number;
}

export interface EventTrendsDTO {
  interval: "hour" | "day";
  from_timestamp: string;
  to_timestamp: string;
  buckets: EventTrendBucketDTO[];
}
